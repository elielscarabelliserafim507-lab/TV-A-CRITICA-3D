...
const sun=new THREE.DirectionalLight(0xfff2da,3.5);sun.position.set(-70,90,45);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);scene.add(sun);

const mk=(c,r=.65,m=.03)=>new THREE.MeshStandardMaterial({color:c,roughness:r,metalness:m});
const M={white:mk(0xf2f3f1),blue:mk(0x063c96,.4,.12),dark:mk(0x06172b,.35,.2),glass:new THREE.MeshPhysicalMaterial({color:0x17334b,roughness:.08,transparent:true,opacity:.68,transmission:.12,clearcoat:1}),floor:mk(0xbfc4c5,.32,.05),road:mk(0x2a2f33,.95),grass:mk(0x3e7e31,1),wood:mk(0x7a5035,.78),metal:mk(0x22282e,.3,.8),red:new THREE.MeshStandardMaterial({color:0xd4202f,emissive:0x74000b,emissiveIntensity:.45}),cyan:new THREE.MeshStandardMaterial({color:0x20d7ee,emissive:0x087f9b,emissiveIntensity:1}),yellow:new THREE.MeshStandardMaterial({color:0xf2c423,emissive:0x8a6400,emissiveIntensity:.35}),pink:mk(0xd98bc6,.4),screen:mk(0x0b1733,.18,.15)};
const solids=[];
function box(s,p,m=M.white,solid=false){const o=new THREE.Mesh(new THREE.BoxGeometry(...s),m);o.position.set(...p);o.castShadow=true;o.receiveShadow=true;scene.add(o);if(solid)solids.push(new THREE.Box3().setFromObject(o));return o}
function cyl(r,h,p,m=M.metal){const o=new THREE.Mesh(new THREE.CylinderGeometry(r,r,h,16),m);o.position.set(...p);o.castShadow=true;scene.add(o);return o}
function sign(t,w=14,h=2,c='#fff',f=90){const cv=document.createElement('canvas');cv.width=1024;cv.height=256;const x=cv.getContext('2d');x.fillStyle=c;x.font=`900 ${f}px Arial`;x.textAlign='center';x.textBaseline='middle';x.fillText(t,512,128);const tx=new THREE.CanvasTexture(cv);tx.colorSpace=THREE.SRGBColorSpace;const o=new THREE.Mesh(new THREE.PlaneGeometry(w,h),new THREE.MeshBasicMaterial({map:tx,transparent:true,side:THREE.DoubleSide}));scene.add(o);return o}
...
function lightPanel(x,y,z,sx=5,sz=1.2){box([sx,.08,sz],[x,y,z],new THREE.MeshStandardMaterial({color:0xffffff,emissive:0xe9f8ff,emissiveIntensity:2}));const l=new THREE.PointLight(0xeaf7ff,2.3,18,2);l.position.set(x,y-.25,z);scene.add(l)}
...
// envelope completo do prédio, estendido para receber todos os estúdios
...
// NOVO ALERTA — primeiro estúdio do corredor, conforme referência da biblioteca
...
// sala reserva no lado direito para expansão futura sem quebrar corredor
const res=room('R',-122,'ESTÚDIO RESERVA','#a8c8ff');box([8,.5,5],[32,.35,-122],M.dark);

// SWITCHER MASTER no final do corredor, ligado fisicamente a ele
...
// CONTROLES DE JOGO
const orbit=new OrbitControls(camera,canvas);orbit.enabled=false;orbit.enableDamping=true;orbit.target.set(5,3,-65);let mode='free',playing=false,yaw=Math.PI,pitch=0,runHeld=false;const keys={};addEventListener('keydown',e=>keys[e.code]=true);addEventListener('keyup',e=>keys[e.code]=false);function lookBy(dx,dy){yaw-=dx*.0027;pitch=Math.max(-1.35,Math.min(1.35,pitch-dy*.0024));camera.rotation.set(pitch,yaw,0,'YXZ')}addEventListener('mousemove',e=>{if(mode==='first'&&document.pointerLockElement===canvas)lookBy(e.movementX,e.movementY)});canvas.onclick=()=>{if(playing&&mode==='first'&&!matchMedia('(pointer:coarse)').matches)canvas.requestPointerLock?.()};
