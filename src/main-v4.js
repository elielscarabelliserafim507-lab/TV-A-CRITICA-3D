...
if (isTouch) {
  ...
  startButton.textContent = 'ENTRAR NA EMISSORA';
...
function setupLookPad() {
  ...
  lookPad.addEventListener('pointermove', (e) => {
    if (e.pointerId !== activeId || !mobileActive) return;
  ...
  lookPad.addEventListener('pointerup', end);
  lookPad.addEventListener('pointercancel', end);
}
...
if (isTouch) {
  ...
  const stopRun = () => { running = false; runButton.classList.remove('active'); };
...
function light(pos, color = 0xffffff, intensity = 20, parent = scene) {
  const l = new THREE.PointLight(color, intensity, isTouch ? 16 : 22, 2);
  l.position.set(...pos);
  parent.add(l);
  return l;
}
...
function createReception() {
  ...
  light([0, 11, -4], 0xeef7ff, isTouch ? 15 : 24, g);
}
...
function createAlerta() {
  ...
  for (const x of [-13, -3, 7, 16]) light([x, 13, 1], 0xdff4ff, isTouch ? 14 : 24, g);
...
function updateMovement(dt) {
  ...
  const speed = (keys.has('ShiftLeft') || running) ? 12 : 7;
  let dx = isTouch ? joyX : 0;
  let dz = isTouch ? joyY : 0;
  if (!isTouch) {
