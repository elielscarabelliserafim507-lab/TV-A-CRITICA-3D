...
scene.fog = new THREE.Fog(0x07111f, 90, 360);
...
scene.add(new THREE.HemisphereLight(0xbad9ff, 0x17202c, 1.4));
...
const MAT = {
  white: new THREE.MeshStandardMaterial({ color: 0xf3f6f9, roughness: 0.58 }),
  dark: new THREE.MeshStandardMaterial({ color: 0x101825, roughness: 0.5 }),
  blue: new THREE.MeshStandardMaterial({ color: 0x0b58c7, roughness: 0.36 }),
  cyan: new THREE.MeshStandardMaterial({ color: 0x20c9ff, roughness: 0.25, emissive: 0x06384c, emissiveIntensity: 0.55 }),
  glass: new THREE.MeshPhysicalMaterial({ color: 0x183b57, transparent: true, opacity: 0.46, roughness: 0.1, metalness: 0.05 }),
  floor: new THREE.MeshStandardMaterial({ color: 0x3b4149, roughness: 0.74 }),
...
function sign(text, w, h, pos, rotY = 0, bg = '#063b93', parent = scene) {
  const mat = new THREE.MeshBasicMaterial({ map: textTexture(text, bg) });
  const m = new THREE.Mesh(new THREE.PlaneGeometry(w, h), mat);
  m.position.set(...pos); m.rotation.y = rotY; parent.add(m); return m;
}

function led(size, pos, color = 0x1bb7ff, parent = scene) {
  const mat = new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: 2.5, roughness: 0.2 });
...
function ceilingLight(pos, color = 0xffffff, intensity = 18, parent = scene) {
  ...
  light.position.set(...pos); parent.add(light);
...
function createReception() {
  ...
  box('SideWall', [0.5, 14, 34], [-29, 7, 0], MAT.white, g);
  box('SideWall', [0.5, 14, 34], [29, 7, 0], MAT.white, g);
  ...
  box('SofaBase', [18, 2.2, 5.5], [-7, 1.1, 6], new THREE.MeshStandardMaterial({ color: 0xd9d5cf, roughness: 0.9 }), g);
  box('SofaBack', [18, 5, 1.3], [-7, 3.5, 8], new THREE.MeshStandardMaterial({ color: 0xd9d5cf, roughness: 0.9 }), g);
  box('CoffeeTable', [7, 1.3, 4], [-7, 0.9, -1], MAT.wood, g);
...
function createCorridorAndRooms() {
  ...
  studioNames.forEach((name, i) => {
    ...
    const screenX = x;
    const screen = sign(name.replace(' ','\n'), 18, 8, [screenX,6,z-9.98], 0, '#073e91', room);
